...
const main = async () => {
  ...
  // The bot reference lap. Two things matter: that it is shown for a course
  // that has one, and that the video is not fetched until someone asks for it —
  // the whole reason it is not bundled.
  const reference = await evaluate(`(async () => {
    const { racePanel, tabs, scene } = globalThis.__fpvsim;
    tabs.show('fly');
    // Onto a map that has one. The default is Race vibes, which the machine
    // pilot cannot fly yet, so leaving it there would only ever exercise the
    // empty case — and the interesting assertions are about the video.
    const sel = document.querySelectorAll('#scene-view select')[0];
    const names = [...sel.options].map((o) => o.textContent);
    const idx = names.indexOf('Circle');
    if (idx >= 0) {
      sel.value = String(idx);
      sel.dispatchEvent(new Event('change', { bubbles: true }));
      await new Promise((r) => setTimeout(r, 300));
    }
    const host = document.querySelector('.race-reference');
    const video = host?.querySelector('video');
    return {
      course: racePanel.race.course.name,
      heading: host?.querySelector('.race-ref-head')?.textContent ?? '',
      values: [...(host?.querySelectorAll('.fl-num-val') || [])].map((n) => n.textContent),
      video: video
        ? {
            preload: video.preload,
            src: video.getAttribute('src'),
            // readyState HAVE_NOTHING and an empty buffered range is what
            // "no media data has been fetched" actually means. networkState
            // settles at NETWORK_IDLE once a source is *selected*, which is
            // not the same thing and is what the first version wrongly
            // asserted against.
            readyState: video.readyState,
            buffered: video.buffered.length,
          }
        : null,
      saysItIsNotALimit: /not a limit|Beating it/.test(host?.textContent ?? ''),
    };
  })()`);
  const hasRef = reference.values.length > 0;
  check(
    'the bot reference lap is shown for a course that has one',
    reference.heading === 'Bot reference flight' &&
      (hasRef ? reference.values.length === 3 : true),
    hasRef
      ? `${reference.course}: ${reference.values.join(', ')}`
      : `${reference.course} has no reference lap yet, and says so`,
  );
  if (hasRef) {
    check(
      'and its video is not fetched until a pilot presses play',
      reference.video !== null &&
        reference.video.preload === 'none' &&
        reference.video.src.startsWith('/reference/') &&
        reference.video.readyState === 0 &&
        reference.video.buffered === 0,
      `preload="${reference.video?.preload}", ${reference.video?.src}, ` +
        `readyState ${reference.video?.readyState} and ${reference.video?.buffered} buffered ranges — no media data fetched`,
    );
    check(
      'and it is presented as a reference rather than a limit',
      reference.saysItIsNotALimit === true,
      'a machine time with no provenance invites the wrong reading',
    );
  }
