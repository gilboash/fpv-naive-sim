...
export function parseBlackboxBinary(
  ...
  //
  // No sign flips here any more. This model now uses Betaflight's pitch
  // convention — positive is nose-down — so a log's axes are already in our
  // units and negating them would put them back out.
  //
  // The convention was established from the logs rather than the source, and
  // the method is worth keeping: a PID in the loop makes motor *commands* an
  // unreliable witness, so correlating the command differential against pitch
  // acceleration says one thing while the eRPM-derived *thrust* differential
  // says the opposite. Rotor speed to thrust to torque has no controller in
  // it. That chain gave r = -0.50 against Betaflight's pitch when this model
  // was nose-up positive, against +0.49 for roll on the same flight.
  for (let ax = 0; ax < 3; ax++) {
  ...
  assumptions.push('pitch taken as-is: this model shares Betaflight\'s nose-down-positive convention');
